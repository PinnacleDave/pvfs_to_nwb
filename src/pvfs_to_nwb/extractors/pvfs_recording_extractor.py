...
def _select_channels_for_rate(
    ...
    if sampling_rate_hz is None:
        rate_counts = Counter(
            rates[name] for name in candidate_names if rates[name] is not None
        )
        if not rate_counts:
    ...
    if not selected:
        raise ValueError(
            f"No channels match the requested sampling rate {sampling_rate_hz} Hz."
        )
...
class PvfsRecordingExtractor(BaseRecording):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # into every extractor constructor; we accept-and-ignore unknown kwargs so
        # the extractor plugs in without an explicit signature change.
