...
and our :class:`PvfsRecordingExtractor`.  The interface picks a single sampling
rate group (the most common rate in the PVFS by default) and exposes it as one
``ElectricalSeries``.  To convert multi-rate PVFS files, instantiate one
interface per rate inside :class:`pvfs_to_nwb.PvfsNWBConverter`.
"""
...
class PvfsRecordingInterface(BaseRecordingExtractorInterface):
    ...
    def _apply_channel_properties(self) -> None:
        ...
        ``add_recording_to_nwbfile`` helper places every channel under our
