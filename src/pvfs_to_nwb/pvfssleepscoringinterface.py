...
inside ``experiment.db3`` (see :mod:`pvfs_to_nwb._metadata` for full schema
notes).  Each populated scoring session is a dense, regular sequence of
fixed-length epochs (10 s by default) with an integer ``score`` indexed
against a per-session legend (``1 -> "Wake"``, ``2 -> "Non REM"``, ...).
...
class PvfsSleepScoringInterface(BaseDataInterface):
    ...
    def has_scoring(self) -> bool:
        ...
        Cheap enough to call from :class:`PvfsNWBConverter` to decide whether
        to attach the interface at all.
        """
